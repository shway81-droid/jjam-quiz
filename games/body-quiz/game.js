/* games/body-quiz/game.js — 패턴 A (4지선다 동시반응) — 우리 몸 퀴즈 */
'use strict';
...
// ── Data: 우리 몸의 부분 (설명 → 부위), 34문제 ────────────────
const ALL_WORDS = [
  { clue: '냄새를 맡는 곳이에요',                 part: '코' },
  { clue: '소리를 듣는 곳이에요',                 part: '귀' },
  { clue: '사물을 보는 곳이에요',                 part: '눈' },
  { clue: '맛을 느끼는 곳이에요',                 part: '혀' },
  { clue: '음식을 씹는 단단한 것이에요',           part: '이' },
  { clue: '말을 하고 음식을 먹는 곳이에요',         part: '입' },
  { clue: '물건을 잡는 곳이에요',                 part: '손' },
  { clue: '신발을 신고 땅을 딛는 곳이에요',         part: '발' },
  { clue: '생각을 하는 곳이에요',                 part: '뇌' },
  { clue: '피를 온몸으로 보내는 곳이에요',         part: '심장' },
  { clue: '숨 쉴 때 공기가 들어가는 곳이에요',       part: '폐' },
  { clue: '먹은 음식을 소화시키는 곳이에요',         part: '위' },
  { clue: '연필을 쥐고 글씨를 쓰는 것이에요',       part: '손가락' },
  { clue: '팔을 굽혔다 펴는 관절이에요',           part: '팔꿈치' },
  { clue: '다리를 굽히는 관절이에요',             part: '무릎' },
  { clue: '머리와 몸을 이어 주고 좌우로 돌려요',     part: '목' },
  { clue: '손끝에 나는 단단한 것이에요',           part: '손톱' },
  { clue: '눈 위에 나서 땀을 막아 줘요',           part: '눈썹' },
  { clue: '몸을 지탱하는 단단한 것이에요',         part: '뼈' },
  { clue: '팔과 몸통을 이어 주는 곳이에요',         part: '어깨' },
  { clue: '손과 팔을 이어 주는 관절이에요',         part: '손목' },
  { clue: '발과 다리를 이어 주는 관절이에요',       part: '발목' },
  { clue: '입 둘레에 있고 붉은 곳이에요',           part: '입술' },
  { clue: '웃으면 발그레해지는 얼굴 양쪽이에요',     part: '볼' },
  { clue: '얼굴 아래쪽, 말할 때 움직여요',         part: '턱' },
  { clue: '배 한가운데 있어요',                   part: '배꼽' },
  { clue: '힘을 쓰면 단단해지는 것이에요',         part: '근육' },
  { clue: '피가 흐르는 가느다란 관이에요',         part: '핏줄' },
  { clue: '발 끝에 다섯 개 있어요',               part: '발가락' },
  { clue: '오줌을 걸러 만드는 곳이에요',           part: '콩팥' },
  { clue: '걸을 때 몸을 받치고 움직이는 두 개예요',  part: '다리' },
  { clue: '머리카락이 자라는 몸의 맨 위예요',       part: '머리' },
  { clue: '음식을 삼키면 지나가는 배 속의 긴 관이에요', part: '창자' },
  { clue: '몸 앞쪽, 밥을 먹으면 볼록해져요',        part: '배' },
];

const ALL_PARTS = ALL_WORDS.map(w => w.part);
...
// 4지선다: 정답 부위 + 다른 부위 3개 (모두 서로 다름)
function makeChoices(cur) {
  const wrong = shuffle(ALL_PARTS.filter(p => p !== cur.part)).slice(0, 3);
  return shuffle([cur.part, ...wrong]);
}
...
function handleAnswerTap(playerIdx, chosenName, btn) {
  ...
  const correct = (chosenName === currentWord.part);
...
function resolveRound(winnerIdx) {
  ...
  getAnswerBtns(winnerIdx).forEach(btn => {
    if (btn.dataset.choice === currentWord.part) {
  ...
  problemStatus.textContent = `${winnerLabel} 정답! (${currentWord.part})`;

  roundLog.push({
    clue: currentWord.clue,
    part: currentWord.part,
    winnerIdx,
    dqPlayers: [...dqSet],
    timedOut: false,
  });
...
function handleTimeout() {
  ...
  problemStatus.textContent = `시간 초과! 정답: ${currentWord.part}`;
...
function loadRound() {
  ...
  wordDisplay.textContent = currentWord.clue;
...
function showResult() {
  ...
  roundLog.forEach((log, idx) => {
    ...
    let cells = `<td style="text-align:left;font-size:0.78rem;">${idx + 1}. ${log.clue} → ${log.part}</td>`;
