...
function generateQuestion() {
    // three opposite pairs, each summing to 7
    const pairs = shuffle([[1,6],[2,5],[3,4]]);
    // assign axes: pair0 -> top/bottom, pair1 -> left/right, pair2 -> front/back
    function place(pair){ return Math.random() < 0.5 ? [pair[0], pair[1]] : [pair[1], pair[0]]; }
    const [top, bottom]   = place(pairs[0]);
    const [left, right]   = place(pairs[1]);
    const [front, back]   = place(pairs[2]);
    const faces = { top, bottom, left, right, front, back };
    const slots = ['top','left','front','right','bottom','back'];
    const blank = slots[rand(0, slots.length - 1)];
    const oppMap = { top:'bottom', bottom:'top', left:'right', right:'left', front:'back', back:'front' };
    const answer = 7 - faces[oppMap[blank]];
    // choices: answer + 3 other distinct numbers from 1..6
    const others = [1,2,3,4,5,6].filter(n => n !== answer);
    shuffle(others);
    const choices = shuffle([answer, others[0], others[1], others[2]]);
    // render the plus net in a 3-col x 4-row grid:
    //  .   top   .
    // left front right
    //  .  bottom .
    //  .  back   .
    function face(name){
      if (name === '') return '<span class="net-cell empty"></span>';
      const val = name === blank ? '?' : faces[name];
      const cls = name === blank ? 'net-cell face blankface' : 'net-cell face';
