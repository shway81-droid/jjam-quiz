/* games/division-quiz/game.js — 패턴 A (4지선다 동시반응) — 나눗셈 몫 맞히기 */
'use strict';
...
// ── Problem Data: 나눗셈 몫 (나누어떨어지는 식만) ─────────────
// 몫(정답)은 항상 하나로 확정 → 보기 모호성 없음.
// 나누는 수 2~9 × 몫 2~9 를 조합해 서로 다른 식을 생성한다(64문제).
const ALL_PROBLEMS = (function () {
  const list = [];
  const seen = new Set();
  for (let divisor = 2; divisor <= 9; divisor++) {
    for (let quotient = 2; quotient <= 9; quotient++) {
      ...
      seen.add(key);
      list.push({ question: dividend + ' ÷ ' + divisor, answer: quotient });
    }
  }
  return list;
})();
...
// 4지선다: 정답(몫) + 정답 근처의 서로 다른 오답 3개. 몫은 유일하므로 오답과 겹치지 않는다.
function makeChoices(problem) {
  const correct = problem.answer;
  const used = new Set([correct]);
  const wrong = [];

  // 정답 근처(±1~±4) 후보를 우선 채운다.
  const nearOffsets = shuffle([-2, -1, 1, 2, -3, 3, -4, 4]);
  for (const off of nearOffsets) {
    const v = correct + off;
    if (v >= 1 && v <= 12 && !used.has(v)) {
      used.add(v);
      wrong.push(v);
      if (wrong.length === 3) break;
    }
  }
  // 혹시 부족하면 1~12 범위에서 채운다.
  let v = 1;
  while (wrong.length < 3 && v <= 12) {
    if (!used.has(v)) { used.add(v); wrong.push(v); }
    v++;
  }
    ...
    currentChoices.forEach((val) => {
      ...
      btn.innerHTML = `<svg viewBox="0 0 110 44" xmlns="http://www.w3.org/2000/svg">
        ...
        <text x="55" y="28" text-anchor="middle" dominant-baseline="middle"
              ...
              font-size="20" font-weight="800" fill="#222">${val}</text>
      </svg>`;
...
function handleAnswerTap(playerIdx, chosenVal, btn) {
  ...
  const correct = (Number(chosenVal) === currentProblem.answer);
...
function resolveRound(winnerIdx) {
  ...
  getAnswerBtns(winnerIdx).forEach(btn => {
    if (Number(btn.dataset.choice) === currentProblem.answer) {
