/* games/instrument-quiz/game.js — 패턴 A (4지선다 동시반응) — 악기 종류 맞히기 */
'use strict';
...
// 4가지 종류(보기). 각 악기는 초등 수준에서 종류가 하나로 확정된다 → 보기 모호성 없음.
const TYPES = ['현악기', '관악기', '타악기', '건반악기'];

// ── Instrument Data: 32종 (국악기 + 서양악기) ────────────────
//   question = 악기(이모지 있으면 함께 표시), answer = 종류(위 TYPES 중 하나)
const ALL_PROBLEMS = [
  { question: '가야금',        answer: '현악기' },
  { question: '거문고',        answer: '현악기' },
  { question: '해금',          answer: '현악기' },
  { question: '아쟁',          answer: '현악기' },
  { question: '🎻 바이올린',   answer: '현악기' },
  { question: '첼로',          answer: '현악기' },
  { question: '🎸 기타',       answer: '현악기' },
  { question: '하프',          answer: '현악기' },
  { question: '대금',          answer: '관악기' },
  { question: '단소',          answer: '관악기' },
  { question: '피리',          answer: '관악기' },
  { question: '태평소',        answer: '관악기' },
  { question: '플루트',        answer: '관악기' },
  { question: '클라리넷',      answer: '관악기' },
  { question: '🎷 색소폰',     answer: '관악기' },
  { question: '🎺 트럼펫',     answer: '관악기' },
  { question: '트롬본',        answer: '관악기' },
  { question: '리코더',        answer: '관악기' },
  { question: '하모니카',      answer: '관악기' },
  { question: '장구',          answer: '타악기' },
  { question: '북',            answer: '타악기' },
  { question: '꽹과리',        answer: '타악기' },
  { question: '징',            answer: '타악기' },
  { question: '소고',          answer: '타악기' },
  { question: '🥁 드럼',       answer: '타악기' },
  { question: '실로폰',        answer: '타악기' },
  { question: '탬버린',        answer: '타악기' },
  { question: '트라이앵글',    answer: '타악기' },
  { question: '캐스터네츠',    answer: '타악기' },
  { question: '🎹 피아노',     answer: '건반악기' },
  { question: '오르간',        answer: '건반악기' },
  { question: '🪗 아코디언',   answer: '건반악기' },
];
...
// 4지선다: 정답 종류 + 나머지 3종류. 종류가 4개뿐이라 항상 4개 보기(정답 유일).
...
function populateAnswerBtns() {
  for (let i = 0; i < playerCount; i++) {
    ...
    currentChoices.forEach((val) => {
      ...
      btn.setAttribute('aria-label', `P${i + 1} ${val}`);
      ...
      btn.innerHTML = `<svg viewBox="0 0 110 44" xmlns="http://www.w3.org/2000/svg">
        ...
        <text x="55" y="28" text-anchor="middle" dominant-baseline="middle"
              ...
              font-size="15" font-weight="800" fill="#222">${val}</text>
      </svg>`;

      onTap(btn, () => handleAnswerTap(i, val, btn));
...
// ── Answer tap handler ───────────────────────────────────────
function handleAnswerTap(playerIdx, chosenVal, btn) {
  ...
  const correct = (chosenVal === currentProblem.answer);
...
function resolveRound(winnerIdx) {
  ...
  getAnswerBtns(winnerIdx).forEach(btn => {
    if (btn.dataset.choice === currentProblem.answer) {
...
function handleTimeout() {
  ...
  problemStatus.textContent = `시간 초과! 정답: ${currentProblem.answer}`;
...
function loadRound() {
  ...
  currentChoices = makeChoices(currentProblem);
  dqSet        = new Set();

  questionCounter.textContent = `${roundIdx + 1} / ${TOTAL_ROUNDS}`;
  wordDisplay.textContent = currentProblem.question;
...
function showResult() {
  ...
  roundLog.forEach((log, idx) => {
    ...
    let cells = `<td style="text-align:left;font-size:0.82rem;">${idx + 1}. ${log.question} → ${log.answer}</td>`;
