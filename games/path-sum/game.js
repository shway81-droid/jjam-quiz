...
function generateQuestion() {
    ...
    if (!path) path = buildPath(3, 5);
    // assign a number 1..9 to each path cell
    const nums = {};
    let sum = 0;
    path.forEach((cell, i) => { const v = rand(1, 9); nums[cell] = v; sum += v; });
    const answer = sum;
    const floor = new Set(path);
    const start = path[0];
    // wrong choices: near the sum, distinct, >=1
    const pool = [];
    for (let n = Math.max(1, answer - 5); n <= answer + 5; n++) { if (n !== answer) pool.push(n); }
    shuffle(pool);
    const choices = shuffle([answer, pool[0], pool[1], pool[2]]);
    let cells = '';
    for (let i = 0; i < GRID_N * GRID_N; i++) {
      if (floor.has(i)) {
        const cls = i === start ? 'sum-cell start' : 'sum-cell num';
        cells += `<span class="${cls}">${nums[i]}</span>`;
      } else {
        cells += `<span class="sum-cell blank"></span>`;
      }
    }
    const html = `<div class="sum-grid">${cells}</div>`;
