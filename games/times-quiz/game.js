...
// ── Question pool: 구구단 2~9단 (64문제) ─────────────────────
// { q: 'x × y = ?', a: 정답(곱), o: [오답 3개] } — 오답은 흔한 실수(한 줄·한 칸 어긋난 곱),
// 모두 양수이고 서로 다르며 정답과 다르도록 만들어 정답이 하나로 확정된다.
function buildTimesPool() {
  const pool = [];
  for (let x = 2; x <= 9; x++) {
    for (let y = 2; y <= 9; y++) {
      const ans = x * y;
      // 그럴듯한 오답 후보: 곱셈표에서 한 줄/한 칸 어긋난 값 + 근처 값
      const cand = [ans + x, ans - x, ans + y, ans - y, ans + 1, ans - 1, ans + 2, ans - 2, ans + 10];
      const wrongs = [];
      for (const c of cand) {
        if (c > 0 && c !== ans && wrongs.indexOf(c) === -1) wrongs.push(c);
        if (wrongs.length === 3) break;
      }
      pool.push({ q: `${x} × ${y} = ?`, a: String(ans), o: wrongs.map(String) });
    }
  }
  return pool;
}
const QUESTION_POOL = buildTimesPool();
