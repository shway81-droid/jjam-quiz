/* ===================================================================
   ...
   ===================================================================
   짬짬이 퀴즈는 짬짬이 게임에서 갈라져 나온 저장소라, 아래 파일들은 두 곳에서
   글자 하나까지 같아야 한다. 지금까지는 디자인을 바꿀 때마다 사람이 양쪽에
   같은 수정을 두 번씩 해 왔고, 한쪽만 고치면 조용히 어긋났다.
   ...
   공통 파일을 고칠 때 순서
     1. jjam 에서 고치고 머지한다
     2. 이 저장소에서 `npm run sync:shared` → 커밋
...
// 두 저장소에서 동일해야 하는 파일.
// (여기에 없는 파일은 저장소마다 다른 것이 정상 — engine.js·index.html·
...
async function fetchUpstream(rel) {
  ...
  if (!res.ok) throw new Error(`상류에서 ${rel} 을 받지 못했습니다 (HTTP ${res.status})`);
...
if (CHECK && problems) {
  console.error(
    ...
    `   두 사이트의 디자인·검증 로직이 갈라진 상태입니다.\n\n` +
    `   고치는 법:\n` +
    `     • 이 저장소를 맞추려면        →  npm run sync:shared\n` +
